...
export class GamesPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get(`${API_PATH}/api/get_start_state`).subscribe((data: any) => {
  ...
  getClickerBall() {
    if (Array.isArray(this.userService.user.clicker)) {
      ...
      this.userService.user.clicker.forEach(
        (item: any) => (result += item.ball)
      );
  ...
  getVictoryBall() {
    if (Array.isArray(this.userService.user.victory)) {
      ...
      this.userService.user.victory.forEach(
        (item: any) => (result += item.ball)
      );
