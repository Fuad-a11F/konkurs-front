...
export class UserService {
    ...
    getUser(password: string) {
        ...
        return this.http.get(`${API_PATH}/api/login_user`, {params})
        .pipe(tap((data: any) => {
            if (data != 'error') {
    ...
    getUserAgain(id: string) {
        ...
        return this.http.get(`${API_PATH}/api/login_user_again`, {params})
        .pipe(tap((data: any) => {
            if (data != 'error') {
                this.user = data
            }
    ...
    addClicker(ball: number) {
        return this.http.post(`${API_PATH}/api/add_clicker`, {ball, id: localStorage.getItem('id')})
                    .pipe(tap((data: any) => {
    ...
    changeIsShow() {
        return this.http.put(`${API_PATH}/api/change_is_show`, {id: localStorage.getItem('id')})
       
    }

    addVictory(victory: string) {
        return this.http.post(`${API_PATH}/api/add_victory`, {victory, id: localStorage.getItem('id')})
            .pipe(tap((data: any) => {
                this.user.victory.push({ball: 0, correct: 0, victory: data.victory})
            }))
    }

    updateVictory(subject: string, form: any) { 
        return this.http.put(`${API_PATH}/api/check_victory_answer`, 
